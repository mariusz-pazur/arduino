...
uint16_t noiseReadsBufferLength = 300;
uint16_t noiseBufferIndex = 0;
uint8_t noiseReadsBuffer[300];
uint32_t noiseSensorReadsDelayInMillis = 60000 / noiseReadsBufferLength;

static struct pt ptMain, ptDHT, ptNoise, ptLED;
static int protothreadMain(struct pt *pt, int interval);
static int protothreadDHT(struct pt *pt, int interval);
static int protothreadNoise(struct pt *pt, int interval);
static int protothreadLED(struct pt *pt, int interval);
...
void loop() 
{
  protothreadMain(&ptMain, 10); 
  protothreadDHT(&ptDHT, dhtSensorReadsDelayInMillis); 
  protothreadNoise(&ptNoise, noiseSensorReadsDelayInMillis);
  protothreadLED(&ptLED, 10);
}
...
void checkForCommandArrived()
{
  if(!Mirf.isSending() && Mirf.dataReady())
  {
    ...
    if (command[1] == 2)//RemoteLedEnv
    {
      ...
      else if (command[2] == 2 || command[2] == 3) //LEDs Effect || LEDs Set color
      {
        ledState[0] = command[2];
